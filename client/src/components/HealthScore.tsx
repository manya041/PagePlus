...
export const HealthScore: React.FC<HealthScoreProps> = ({ result }) => {
  ...
  // Determine Top 3-4 Issues Detected
  const topIssues: string[] = [];
  if (!result.metaDescription) topIssues.push('Missing Meta Description');
  if (result.h1Count === 0) topIssues.push('Missing H1 Heading');
  else if (result.h1Count > 1) topIssues.push('Multiple H1 Headings');

  if (result.status >= 400) topIssues.push(`HTTP ${result.status} Response`);
  if (result.wordCount < 300) topIssues.push('Low Visible Content Density');
  if (result.responseTime > 200 && topIssues.length < 4) topIssues.push(`Slow Server Response (${result.responseTime}ms)`);
  if (result.missingAltImages > 0 && topIssues.length < 4) topIssues.push(`${result.missingAltImages} Images Missing Alt Text`);

  const displayIssues = topIssues.slice(0, 4);
    ...
    >
      <div className="flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="space-y-3 text-center md:text-left flex-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 pt-1 text-[11px] font-semibold text-accent-subtle">
      ...
      {/* Compact Summary Panel: Top Issues Detected */}
      <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2 font-bold text-accent shrink-0">
          <AlertCircle className="w-4 h-4 text-amber-500" />
          <span>Top Issues Detected:</span>
        </div>
        ...
        {displayIssues.length > 0 ? (
          <div className="flex flex-wrap items-center gap-2">
            {displayIssues.map((issue) => (
              <span
                key={issue}
                className="px-2.5 py-1 bg-amber-50 text-amber-800 border border-amber-200 font-semibold rounded-lg text-[11px] flex items-center gap-1"
              >
                <span>•</span>
                <span>{issue}</span>
              </span>
          ...
          <div className="px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 font-semibold rounded-lg text-xs flex items-center gap-1.5">
